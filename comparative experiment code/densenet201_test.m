...
%%
%模型构建
% lgraph = imagePretrainedNetwork("inceptionresnetv2",Weights="none");
lgraph = densenet201(Weights="none");
% lgraph = layerGraph(net);
analyzeNetwork(lgraph)
inputSize = lgraph.Layers(1).InputSize
lgraph = disconnectLayers(lgraph,"input_1","conv1|conv");
...
lgraph = connectLayers(lgraph,"concat","conv1|conv");
...
%分类层指定网络的输出类别。用一个没有类别标签的新分类层替换分类层。
newClassLayer = classificationLayer('Name','FND_classoutput');
...
save('trained_model_densenet201.mat', 'net');
%%
%验证测试数据
% 确保网络处于评估模式
net = resetState(net);
...
% accuracy = 0.9134 0.8267 0.9097 0.8845 0.8700 0.8953 0.8917 0.8809 0.8953 0.8989
% 参数 = 18.1M 711