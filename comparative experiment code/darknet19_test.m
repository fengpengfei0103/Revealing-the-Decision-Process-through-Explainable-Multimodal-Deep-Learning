...
%%
%模型构建
net = darknet19(Weights="none");
lgraph = layerGraph(net);
analyzeNetwork(lgraph)
inputSize = lgraph.Layers(1).InputSize
lgraph = disconnectLayers(lgraph,"input","conv1");
...
newLearnableLayer = convolution2dLayer([1 1],numClasses,'Stride' ,1,"Name","new_conv","Padding","same");  
lgraph = replaceLayer(lgraph,'conv19',newLearnableLayer);
newavgLearnableLayer = averagePooling2dLayer(7,'Stride' ,1,"Name","new_avg");  
lgraph = replaceLayer(lgraph,'avg1',newavgLearnableLayer);
...
save('trained_model_darknet19.mat', 'net');
%%
%验证测试数据
% 确保网络处于评估模式
net = resetState(net);
...
% accuracy = 0.8953 0.9134 0.8917 0.9025 0.9061 0.9097 0.8881 0.8773 0.8917 0.8953
% time = 03：15
% 参数 = 19.8M 67